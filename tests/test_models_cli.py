...
def test_models_list_uses_builtin_fallback(monkeypatch: MagicMock, tmp_path: Path, capsys: MagicMock) -> None:
    ...
    assert payload["profiles"]["wan22-t2v"]["supports"] == ["videogen.wan22-t2v"]
    ...
    assert payload["profiles"]["wan22-dasiwa-tastysin-t2v"]["architecture"] == "wan22"
    assert payload["profiles"]["wan22-dasiwa-boundbite-i2v"]["architecture"] == "wan22"
    assert payload["profiles"]["wan22-dasiwa-boundbite-t2v"]["architecture"] == "wan22"
...
def test_models_init_and_show(monkeypatch: MagicMock, tmp_path: Path, capsys: MagicMock) -> None:
    ...
    assert payload["resolved_defaults"]["videogen.wan22-t2v"]["profile"] == "wan22-t2v"
