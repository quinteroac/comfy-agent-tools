...
description: Generate local MiniMax H3 videos with synchronized native audio from one or more reference images using comfy-videogen. Use when the user wants local MiniMax H3 reference-to-video generation. Do not use for MiniMax hosted APIs, Seedance, image-only generation, or ComfyUI server workflows.
---

# comfy-minimax-videogen

Use this skill for local MiniMax H3 reference-to-video generation through the
`comfy-videogen` CLI. The pipeline is included in `comfy-diffusion` 2.6.0 and
generates an MP4 with synchronized audio.
...
The default profile is `minimax-h3`, architecture `minimax-h3`, and capability
